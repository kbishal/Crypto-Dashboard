...
const CoinDetail = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-white px-6 py-10">
      ...
      <button
        onClick={() => navigate("/")}
        className="mb-4 px-4 py-2 rounded bg-gray-700 hover:bg-gray-600 text-sm text-white font-medium transition"
      >
        ← Back to Dashboard
      </button>
      {/* Header */}
      <div className="flex items-center gap-4 mb-6">
        ...
        <span className="ml-2 bg-yellow-600 text-xs font-bold px-2 py-1 rounded">HOT</span>
      </div>

      {/* Price summary */}
      <div className="mb-8 text-lg">
        <span className="text-gray-300">1 {coin.symbol.toUpperCase()} = </span>
        <span className="text-white font-semibold text-xl">
          ${coin.market_data.current_price.usd.toLocaleString()}
        </span>
        <span className={`ml-2 font-medium ${coin.market_data.price_change_percentage_24h >= 0 ? "text-green-500" : "text-red-500"
          }`}>
